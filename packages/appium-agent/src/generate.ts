import type { MobileCoverageGap, AppiumTestFile } from './types.js';

function toTestPath(sourcePath: string, screenName: string): string {
  return `e2e/${screenName}.appium.test.ts`;
}

function generateScreenTest(gap: MobileCoverageGap): string {
...
function generateNavigationTest(gap: MobileCoverageGap): string {
  const { screen_name } = gap;
...
export function generateAppiumTests(gaps: MobileCoverageGap[]): AppiumTestFile[] {
  return gaps.map((gap) => {
    const content =
      gap.gap_type === 'missing_navigation_test'
