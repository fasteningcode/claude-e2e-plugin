import type { MobileCoverageGap, MobileRepoAnalysis } from './types.js';

export interface FileEntry {
  path: string;
}

function detectFramework(paths: string[]): MobileRepoAnalysis['framework'] {
  if (paths.some((p) => p.endsWith('.dart'))) return 'flutter';
  if (
    paths.some(
      (p) => p.endsWith('.swift') || p.endsWith('.m') || p.endsWith('.xib') || p.endsWith('.storyboard'),
    )
  )
    return 'native-ios';
  if (paths.some((p) => p.endsWith('.kt') || p.endsWith('.java'))) return 'native-android';
  if (
    paths.some(
      (p) =>
        p.includes('android/') &&
    ...
    return 'react-native';
  if (paths.some((p) => p.endsWith('.tsx') || p.endsWith('.jsx'))) return 'react-native';
  return 'unknown';
}
...
function isScreenFile(path: string): boolean {
  ...
  return (
    (lower.includes('screen') || lower.includes('view') || lower.includes('page')) &&
...
export function analyzeMobileRepo(files: FileEntry[]): MobileRepoAnalysis {
