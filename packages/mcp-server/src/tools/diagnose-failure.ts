...
import type { DiagnoseFailureInput, DiagnosisReport } from '../types.js';
...
export async function handleDiagnoseFailure(
  ...
  const affectedFiles = [...new Set([...logs.matchAll(FILE_PATTERN)].map((m) => m[1] ?? ''))].filter(
    Boolean,
  );
...
function inferRootCause(logs: string, errors: string[]): string {
...
function inferFix(rootCause: string, _logs: string): string {
