...
export async function handleAnalyzeRepo(
  ...
  client: GitHubClient,
): Promise<{ gaps: CoverageGap[]; summary: string }> {
  ...
  for (const sourcePath of sourceFiles) {
    ...
    const hasTest =
      testFiles.has(expectedTestPath) ||
      [...testFiles].some(
        (t) =>
          t.includes(sourcePath.split('/').pop()?.replace(/\.(ts|tsx|js|jsx)$/, '') ?? '') &&
          t !== sourcePath,
      );

    if (!hasTest) {
      const isHighPriority =
        sourcePath.includes('page') ||
  ...
  const summary =
    `Found ${gaps.length} coverage gap(s) across ${sourceFiles.length} source file(s). ` +
